...
setup_rprofile_auto_start <- function() {
  ...
  tryCatch({
    ...
    auto_start_lines <- c(
      ...
      "        })",
      "      } else {",
      "        cat(\"DEBUG .First(): RStudio not ready, triggering callback...\\n\")",
      "        # RStudio not ready yet - trigger callback by executing a dummy command",
      "        tryCatch({",
      "          invisible(1 + 1)",
      "        }, error = function(e) {",
      "          cat(\"DEBUG .First(): Error triggering callback:\", e$message, \"\\n\")",
      "        })",
      "      }",
      "    }",
      "  } else {",
      ...
      "    tryCatch(rstudioai::auto_start_rgent(), error = function(e) NULL)",
      "  } else {",
      "    cat(\"DEBUG .Rprofile: RStudio not ready, triggering callback with dummy command...\\n\")",
      "    # Trigger callback immediately by executing a dummy command",
      "    tryCatch({",
      "      invisible(1 + 1)",
      "    }, error = function(e) {",
      "      cat(\"DEBUG .Rprofile: Error triggering callback:\", e$message, \"\\n\")",
      "    })",
      "  }",
      "}"
    )
