...
# Helper function to make streaming POST requests with httr
# Uses httr::write_stream() to process chunks as they arrive (like ChatGPT/Gemini)
stream_httr_post <- function(url, body, headers = NULL, timeout = 60, chunk_callback) {
  # Accumulate buffer for incomplete lines (use environment for proper scoping)
  stream_env <- new.env()
  stream_env$line_buffer <- ""  # Use text buffer for simplicity
  stream_env$full_response <- ""
  stream_env$callback_called <- FALSE
  stream_env$chunk_count <- 0
  stream_env$total_bytes <- 0
  
  
  # Create write_stream callback that processes chunks as they arrive
  write_stream_callback <- function(x) {
    # This MUST be called if write_stream is working
    stream_env$callback_called <- TRUE
    stream_env$total_bytes <- stream_env$total_bytes + length(x)
    stream_env$chunk_count <- stream_env$chunk_count + 1
    
    # Store bytes processed - must always return this value
    bytes_processed <- length(x)
    
    # Convert raw bytes to text
    tryCatch({
      chunk_text <- rawToChar(x)
      
      stream_env$line_buffer <- paste0(stream_env$line_buffer, chunk_text)
      
      # Process complete lines
      while (grepl("\n", stream_env$line_buffer, fixed = TRUE)) {
        # Extract first complete line
        newline_pos <- regexpr("\n", stream_env$line_buffer, fixed = TRUE)[1]
        if (newline_pos <= 0) break
        
        line <- substr(stream_env$line_buffer, 1, newline_pos - 1)
        stream_env$line_buffer <- substr(stream_env$line_buffer, newline_pos + 1, nchar(stream_env$line_buffer))
        ...
        if (grepl("^data: ", line)) {
          ...
          if (json_data != "[DONE]") {
            ...
            result <- tryCatch({
              ...
              cat("ERROR in chunk callback:", e$message, "\n")
              cat("ERROR stack trace:", paste(capture.output(traceback()), collapse = "\n"), "\n")
              NULL
            })
            
            
            # Check if callback wants to stop
            if (!is.null(result) && is.list(result) && identical(result$stop, TRUE)) {
              return(0)  # Return 0 to stop reading
            }
          } else {
      ...
      # If conversion fails, try to handle partial UTF-8
      cat("ERROR: Error converting chunk to text:", e$message, "\n")
      cat("ERROR: Raw chunk length:", length(x), "\n")
      # Still return the length so streaming continues
    })
    
    # CRITICAL: Always return the number of bytes processed
    # This tells httr to continue reading
    return(bytes_processed)
  }
  
  # Make POST request with streaming
  # write_stream() must be passed directly as a parameter, not in config
  
  tryCatch({
    # Build the POST call with write_stream and other options
    # write_stream needs to be a direct argument, not in config
    post_args <- list(
      url = url,
      body = body,
      encode = "json",
      httr::timeout(timeout),
      httr::write_stream(write_stream_callback)
    )
    
    # Add headers if provided
    if (!is.null(headers)) {
      if (is.character(headers) && !is.null(names(headers))) {
        post_args <- c(post_args, list(httr::add_headers(.headers = headers)))
      } else {
        post_args <- c(post_args, list(httr::add_headers(headers)))
      }
    }
    
    # Call POST with all arguments
    response <- do.call(httr::POST, post_args)
    
    
    # Debug: Check if callback was called
    if (!stream_env$callback_called) {
      cat("WARNING: write_stream callback was not called. Response may have been buffered.\n")
      
      # Fallback: process response content directly
          ...
          if (grepl("^data: ", line)) {
            ...
            if (json_data != "[DONE]") {
              tryCatch({
                ...
                # Small delay to simulate streaming (only in fallback mode)
                # This helps the frontend receive chunks incrementally
                Sys.sleep(0.01)  # 10ms delay between chunks
              }, error = function(e) {
                cat("ERROR in fallback chunk callback:", e$message, "\n")
                cat("ERROR stack trace:", paste(capture.output(traceback()), collapse = "\n"), "\n")
              })
            }
          }
    ...
    if (nchar(stream_env$line_buffer) > 0) {
      
      if (grepl("^data: ", stream_env$line_buffer)) {
        json_data <- substring(stream_env$line_buffer, 7)
        if (json_data != "[DONE]") {
          tryCatch({
            chunk_callback(json_data)
          }, error = function(e) {
            cat("ERROR processing final chunk:", e$message, "\n")
          })
        }
      }
    }
    ...
    return(list(
      ...
      response = response,
      full_response = stream_env$full_response
    ))
  }, error = function(e) {
    cat("ERROR in stream_httr_post:", e$message, "\n")
    cat("ERROR stack trace:", paste(capture.output(traceback()), collapse = "\n"), "\n")
    return(list(
      success = FALSE,
      error = e$message,
...
#' 
#' Checks if RStudio is available, then starts Rgent automatically.
#' 
...
setup_rprofile_auto_start <- function() {
  tryCatch({
    ...
    if (file.exists(rprofile_path)) {
      ...
      has_hook <- any(grepl("rstudio\\.sessionInit", rprofile_content))
      # If it has our comment AND has the session hook, consider it set up
      if (has_comment && has_rstudioai && has_hook) {
    ...
    auto_start_lines <- c(
      ...
      "        # Always try to start, regardless of whether it's a new or restored session",
      ...
      response <- switch(request$action,
        ...
        "chat_with_ai" = {
          ...
          tryCatch({
            ...
            # Initialize chunk buffer and full response for streaming
            current_chunk_buffer <- ""
            full_response <- ""
            
            # Define chunk callback for streaming
            chunk_callback <- function(json_data) {
              ...
              tryCatch({
                ...
                if (!is.null(chunk_data$chunk)) {
                  ...
                  # Send chunks immediately for true streaming effect
                  # Only buffer if we're in the middle of a code block or incomplete function
                  # Otherwise, send every chunk immediately (even single characters)
                  should_send <- !code_block_open && !incomplete_function
                  
                  # Also send if we've accumulated enough (every 5 chars) even if in code block
                  # This ensures we don't wait too long
                  if (!should_send && nchar(current_chunk_buffer) >= 5) {
                    should_send <- TRUE
                  }
                ...
                cat("ERROR chunk_callback: Error parsing chunk:", e$message, "\n")
                cat("ERROR chunk_callback: json_data was:", substr(json_data, 1, 500), "\n")
                cat("ERROR chunk_callback: Stack trace:", paste(capture.output(traceback()), collapse = "\n"), "\n")
                return(NULL)
              })
            }
            
            # Make streaming POST request
            stream_result <- stream_httr_post(
              url = "https://rgent.onrender.com/chat/stream",
              body = request_body,
            ...
            if (stream_result$success) {
              ...
              cat("Backend returned error status:", stream_result$status_code, "\n")
              if (!is.null(stream_result$error)) {
                cat("Error:", stream_result$error, "\n")
              }
              list(action = "ai_response", message = paste("Error connecting to AI service. Status:", stream_result$status_code))
            }
          }, error = function(e) {
            ...
            if (grepl("timeout", tolower(e$message)) || grepl("timed out", tolower(e$message))) {
              ...
              # Retry with increased timeout using streaming
              tryCatch({
                retry_chunk_buffer <- ""
                retry_full_response <- ""
                
                retry_chunk_callback <- function(json_data) {
                  tryCatch({
                    chunk_data <- jsonlite::fromJSON(json_data)
                    if (!is.null(chunk_data$chunk)) {
                      retry_full_response <<- paste0(retry_full_response, chunk_data$chunk)
                      retry_chunk_buffer <<- paste0(retry_chunk_buffer, chunk_data$chunk)
                      
                      # Send chunks as they arrive
                      if (nchar(retry_chunk_buffer) > 50) {
                        chunk_response <- list(
                          action = "ai_response",
                          streaming = TRUE,
                          chunk = retry_chunk_buffer
                        )
                        ws$send(jsonlite::toJSON(chunk_response, auto_unbox = TRUE))
                        retry_chunk_buffer <<- ""
                      }
                ...
                retry_result <- stream_httr_post(
                  url = "https://rgent.onrender.com/chat/stream",
                  body = request_body,
                  headers = c("Accept" = "text/event-stream"),
                  timeout = 60,
                  chunk_callback = retry_chunk_callback
                )
                
                if (retry_result$success) {
                  if (nchar(retry_full_response) > 0) {
                    add_to_conversation_history("ai", retry_full_response)
                  }
                  finish_response <- list(
                    action = "ai_response",
                    streaming = TRUE,
        ...
        "get_last_error" = {
          ...
          if (!has_error) {
            ...
            tryCatch({
              ...
              debug_chunk_callback <- function(json_data) {
                tryCatch({
                  chunk_data <- jsonlite::fromJSON(json_data)
                  ...
                  } else if (!is.null(chunk_data$chunk)) {
                    chunk_text <- chunk_data$chunk
                  }
                  
                  if (!is.null(chunk_text) && nchar(chunk_text) > 0) {
                    debug_full_response <<- paste0(debug_full_response, chunk_text)
                    debug_chunk_buffer <<- paste0(debug_chunk_buffer, chunk_text)
                    
                    # Check if we're in the middle of a code block
                    code_block_open <- grepl("```[^`]*$", debug_chunk_buffer)
                    incomplete_function <- grepl("^[a-zA-Z_][a-zA-Z0-9_]*\\([^)]*$", debug_chunk_buffer)
                    
                    # Send chunk to frontend when we have a complete word or sentence
                    if (!code_block_open && !incomplete_function && (grepl("[.!?]\\s*$", debug_chunk_buffer) || 
                        grepl("\\n\\n", debug_chunk_buffer) ||
                        grepl("```$", debug_chunk_buffer) ||
                        nchar(debug_chunk_buffer) > 100)) {
                      
                      chunk_response <- list(
                        action = "debug_ai_response",
                        streaming = TRUE,
                        chunk = debug_chunk_buffer
                      )
                      ws$send(jsonlite::toJSON(chunk_response, auto_unbox = TRUE))
                  ...
                  # Check if this is the final chunk
                  if (is_done || (!is.null(chunk_data$action) && chunk_data$action == "ai_response" && !chunk_data$streaming)) {
                    # Final chunk
                    if (nchar(debug_chunk_buffer) > 0) {
              ...
              debug_result <- stream_httr_post(
                url = "https://rgent.onrender.com/chat/stream",
                body = request_body,
                headers = NULL,
                timeout = 30,
                chunk_callback = debug_chunk_callback
              )
              ...
              if (!debug_result$success) {
                ...
                error_response <- list(
                  action = "debug_ai_response",
                  streaming = FALSE,
                  error = paste("AI analysis failed:", debug_result$status_code)
                )
          ...
          tryCatch({
            ...
            if (!analysis_result$success) {
              ...
              tryCatch({
                ...
                # Define chunk callback for plot analysis streaming
                plot_chunk_callback <- function(json_data) {
                  ...
                  tryCatch({
                    ...
                    if (!is.null(chunk_data$chunk)) {
                      plot_full_response <<- paste0(plot_full_response, chunk_data$chunk)
                      plot_chunk_buffer <<- paste0(plot_chunk_buffer, chunk_data$chunk)
                      
                      # Check if we're in the middle of a code block
                      code_block_open <- grepl("```[^`]*$", plot_chunk_buffer)
                      incomplete_function <- grepl("^[a-zA-Z_][a-zA-Z0-9_]*\\([^)]*$", plot_chunk_buffer)
                      
                      # Send chunk to frontend when we have a complete word or sentence
                      if (!.GlobalEnv$operation_stopped && !code_block_open && !incomplete_function && 
                          (grepl("[.!?]\\s*$", plot_chunk_buffer) || 
                           grepl("\\n\\n", plot_chunk_buffer) ||
                           grepl("```$", plot_chunk_buffer) ||
                           nchar(plot_chunk_buffer) > 100)) {
                        
                        chunk_response <- list(
                          action = "ai_response",
                          streaming = TRUE,
                          chunk = plot_chunk_buffer
                        )
                        ws$send(jsonlite::toJSON(chunk_response, auto_unbox = TRUE))
                    ...
                    if (!is.null(chunk_data$done) && chunk_data$done) {
                      ...
                      if (!.GlobalEnv$operation_stopped && nchar(plot_chunk_buffer) > 0) {
                        chunk_response <- list(
                          action = "ai_response",
                          streaming = TRUE,
                          chunk = plot_chunk_buffer
                ...
                # Make streaming POST request
                plot_result <- stream_httr_post(
                  url = "https://rgent.onrender.com/chat/stream",
                  body = request_body,
                  headers = c("Accept" = "text/event-stream"),
                  timeout = 60,
                  chunk_callback = plot_chunk_callback
                )
        ...
        "analyze_visualization_plot" = {
          ...
          if (!analysis_result$success) {
            ...
            tryCatch({
              ...
              # Define chunk callback for plot analysis streaming
              plot2_chunk_callback <- function(json_data) {
                tryCatch({
                  chunk_data <- jsonlite::fromJSON(json_data)
                  if (!is.null(chunk_data$chunk)) {
                    plot2_full_response <<- paste0(plot2_full_response, chunk_data$chunk)
              ...
              # Make streaming POST request
              plot2_result <- stream_httr_post(
                url = "https://rgent.onrender.com/chat/stream",
                body = request_body,
                headers = c("Accept" = "text/event-stream"),
                timeout = 60,
                chunk_callback = plot2_chunk_callback
              )
              
              if (plot2_result$success) {
                ...
                list(action = "visualization_plot_analysis", 
                     success = FALSE, 
                     message = paste("AI analysis failed with status:", plot2_result$status_code))
              }
            }, error = function(e) {
              list(action = "visualization_plot_analysis", 
                   success = FALSE, 
      ...
      if (rstudio_available) {
        ...
        start_callback <- function(expr, value, ok, visible) {
          ...
          tryCatch({
            if (rstudioapi::isAvailable()) {
              ...
              # Remove callback if Rgent started successfully or is already running
              if (isTRUE(result)) {
                tryCatch({
          ...
          return(TRUE)  # Keep callback active if Rgent didn't start
        }
        ...
        delayed_callback <- function(expr, value, ok, visible) {
          ...
          tryCatch({
            if (rstudioapi::isAvailable()) {
              result <- auto_start_rgent()
              # Success - remove callback if Rgent started or is already running
              if (isTRUE(result)) {
                tryCatch({
              ...
              # If Rgent didn't start, keep trying
              return(TRUE)
            } else {
        ...
        simple_callback <- function(expr, value, ok, visible) {
          ...
          tryCatch({
            if (rstudioapi::isAvailable()) {
              result <- auto_start_rgent()
              # Remove this callback once successful or if already running
              if (isTRUE(result)) {
                tryCatch({
          ...
          return(TRUE)  # Keep it active if Rgent didn't start
        }
