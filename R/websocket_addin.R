...
setup_rprofile_auto_start <- function() {
  ...
  tryCatch({
    ...
    auto_start_lines <- c(
      ...
      "    }",
      "  }",
      "  # Try immediately if RStudio is already ready, otherwise poll until ready",
      "  cat(\"DEBUG .Rprofile: Checking if RStudio is ready immediately...\\n\")",
      "  if (requireNamespace(\"rstudioapi\", quietly = TRUE) && rstudioapi::isAvailable()) {",
      "    cat(\"DEBUG .Rprofile: RStudio ready immediately, starting Rgent...\\n\")",
      "    tryCatch(rstudioai::auto_start_rgent(), error = function(e) NULL)",
      "  } else {",
      "    cat(\"DEBUG .Rprofile: RStudio not ready, starting polling loop...\\n\")",
      "    # Poll for RStudio availability (non-blocking with delays)",
      "    .rgent_poll_count <- 0L",
      "    .rgent_max_polls <- 50L",
      "    .rgent_poll <- function() {",
      "      .rgent_poll_count <<- .rgent_poll_count + 1L",
      "      if (.rgent_poll_count <= .rgent_max_polls) {",
      "        if (requireNamespace(\"rstudioapi\", quietly = TRUE) && rstudioapi::isAvailable()) {",
      "          cat(\"DEBUG .Rprofile: RStudio now available (poll\", .rgent_poll_count, \"), starting Rgent...\\n\")",
      "          tryCatch(rstudioai::auto_start_rgent(), error = function(e) NULL)",
      "          tryCatch(removeTaskCallback(\".rgent_poll\"), error = function(e) NULL)",
      "          return(FALSE)",
      "        } else {",
      "          if (.rgent_poll_count %% 5L == 0L) {",
      "            cat(\"DEBUG .Rprofile: Still polling for RStudio... (attempt\", .rgent_poll_count, \")\n\")",
      "          }",
      "          return(TRUE)",
      "        }",
      "      } else {",
      "        cat(\"DEBUG .Rprofile: Max polls reached, giving up. Callback will handle it.\\n\")",
      "        tryCatch(removeTaskCallback(\".rgent_poll\"), error = function(e) NULL)",
      "        return(FALSE)",
      "      }",
      "    }",
      "    addTaskCallback(.rgent_poll, name = \".rgent_poll\")",
